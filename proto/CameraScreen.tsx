...
import { useInferenceLogic, DetectionResult } from './useInferenceLogic';
...
export default function App() {
  ...
  useEffect(() => {
    const prewarm = async () => {
      try {
        ...
        // Pre-warm Share module by checking if it's available
        // This loads the native module without showing any UI
        if (Share.isPackageInstalled) {
          await Share.isPackageInstalled('com.android.mms'); // Just to init the module
        }
        console.log('📤 Share module pre-warmed');
      } catch (e) {
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      if (Platform.OS === 'ios') {
        // iOS: Use base64 data URI directly - more reliable than file paths
        await Share.open({
          url: `data:image/jpeg;base64,${capturedImageUri}`,
          type: 'image/jpeg',
          title: 'Beszkennelt BERRĪ füzet',
          failOnCancel: false,
        });
      } else {
        // Android: Write to cache and share via FileProvider
        const fileName = `berri_scan_${Date.now()}.jpg`;
  ...
  const renderDocumentOverlay = () => {
    ...
    if (!isRectangleGood) {
      ...
      pathStyle = hasLockedQrValue ? 'fill' : 'fill';

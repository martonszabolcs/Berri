...
const HistorySelectScreen = () => {
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              console.log(
                `🗑️ Starting deletion of ${getAllHistories.length} items...`,
              );
  ...
  const mergeSelectedHistories = async () => {
    ...
    try {
      console.log(
        `🔗 Starting merge of ${getAllHistories.length} history items...`,
      );
      ...
      const updatedHistory = history.filter(
        (item: any) => !selectedTimestamps.includes(item.timestamp),
      );
      ...
      console.log(
        `📝 Updated history: removed ${selectedTimestamps.length} items, added 1 merged item`,
      );
  ...
  const shareSelectedHistories = async () => {
    ...
    try {
      ...
      console.log(
        `📤 Sharing ${fileCount} files from ${getAllHistories.length} history entries...`,
      );
  ...
  const resumeBulkResendWithFreshTokens = useCallback(
    async (service: 'dropbox' | 'onedrive', freshSettings: any) => {
      console.log(`🔄 Resuming bulk resend with fresh ${service} tokens...`);

      try {
        // Get the current state of bulk operation
        const currentHistories = selectedCards
          .map(cardId => history.find((item: any) => item.id === cardId))
          .filter(Boolean);
        ...
        console.log(
          `📋 Resuming ${pendingHistories.length} pending ${service} operations...`,
        );
        ...
        for (let i = 0; i < pendingHistories.length; i++) {
          ...
          try {
            console.log(
              `📤 [${i + 1}/${pendingHistories.length}] Resending ${
            ...
            console.log(
              `✅ [${i + 1}/${pendingHistories.length}] Successfully resent ${
                historyItem.fileName
              }`,
            );
        ...
        console.error(
          `❌ Critical error during bulk resend resume for ${service}:`,
          error,
        );
        setIsBulkResending(false);
      }
        ...
        if (response.ok) {
          try {
            ...
            try {
              ...
              console.log(
                '🔄 Resuming bulk resend with fresh Dropbox tokens...',
              );
      ...
      try {
        ...
        if (response.ok) {
          try {
            ...
            try {
              console.log(
                '🔄 Resuming bulk resend with fresh OneDrive tokens...',
              );
              await resumeBulkResendWithFreshTokens(
                'onedrive',
                updatedSettings,
              );
        ...
        <View style={styles.container}>
          ...
          <View style={styles.selectionBottomBar}>
            <TouchableOpacity
              ...
              onPress={() => removeSelectedHistories()}
            >
              <Image
            ...
            <TouchableOpacity
              style={styles.bottomButton}
              onPress={() => mergeSelectedHistories()}
            >
              <Image
            ...
            <TouchableOpacity
              style={styles.bottomButton}
              onPress={() => resendSelectedHistories()}
            >
              <Image
            ...
            <TouchableOpacity
              style={styles.bottomButton}
              onPress={() => shareSelectedHistories()}
            >
              <Image
...
export default HistorySelectScreen;
